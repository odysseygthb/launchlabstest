...
export function useAthletesCount(filters: AthleteFilters = {}) {
    return useQuery({
        queryKey: ['athletes-count', filters.id, filters.name, filters.sport, filters.country, filters.status, filters.gender],
...
export function useAthletes({ filters = {}, page = 0, pageSize = 20 }: QueryOptions = {}) {
    return useQuery({
        queryKey: ['athletes', filters.id, filters.name, filters.sport, filters.country, filters.status, filters.gender, page, pageSize],
        queryFn: () => {
